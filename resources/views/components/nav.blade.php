                            ...
                            <div class="-my-6 divide-y divide-white/10">
                                <div class="space-y-2 py-6">
                                   ...
                                   <a href="{{ route('orders.index') }}" class="-mx-3 block rounded-lg px-3 py-2 text-base/7 font-semibold text-white hover:bg-white/5 {{ request()->is('orders') ? 'bg-black text-white' : ''}}"
                                    ...
                                    <a href="{{ route('cart.show') }}" class="text-white {{ request()->is('/cart') ? 'px-4 py-2 -mt-2 bg-black rounded-2xl' : ''}}">My Cart</a>
                                    @if (! $cartCount == 0)
                                        <div class="top-40 right-22 flex items-center justify-center w-6 h-6 bg-red-700 text-white text-xs font-bold rounded-full border border-amber-50">
                                    {{ $cartCount }}
                                    </div>
                                    @endif
                                    </div>
